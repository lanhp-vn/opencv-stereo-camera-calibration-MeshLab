...
unCalibLeft = cv2.imread('unCalibLeft.png')
unCalibRight = cv2.imread('unCalibRight.png')

# Rectify the images on rotation and alignment
Left_nice= cv2.remap(unCalibLeft,Left_Stereo_Map[0],Left_Stereo_Map[1], interpolation = cv2.INTER_LANCZOS4, borderMode = cv2.BORDER_CONSTANT)
# Rectify the image using the calibration parameters founds during the initialization
Right_nice= cv2.remap(unCalibRight,Right_Stereo_Map[0],Right_Stereo_Map[1], interpolation = cv2.INTER_LANCZOS4, borderMode = cv2.BORDER_CONSTANT)
